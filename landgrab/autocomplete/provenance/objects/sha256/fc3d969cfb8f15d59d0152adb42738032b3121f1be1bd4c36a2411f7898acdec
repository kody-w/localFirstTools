...
class CapabilityPackageTests(unittest.TestCase):
    ...
    def test_changed_source_or_capability_cannot_qualify_as_stable(self):
        self.write_source("sample.txt", b"dirty selected source")
        report = package.qualify(self.args)
        self.assertEqual(report["outcome"], "failed")
        self.assertFalse(report["gates"]["source_matches"])
